...
## 2024-05-26 - Three.js Resize Event Performance
**Learning:** Attaching expensive WebGL context reallocations like `renderer.setSize()` directly to unthrottled `resize` events in Three.js causes continuous main thread blocking and layout thrashing, severely degrading performance while the user is resizing the browser.
**Action:** Always wrap `renderer.setSize()` and associated camera updates (`camera.updateProjectionMatrix()`) bound to `resize` events in a debounce function (e.g., `setTimeout(..., 200)`) to ensure the task only runs once the resize action has stopped.
