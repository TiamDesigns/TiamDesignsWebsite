...
## 2024-05-24 - Layout Thrashing in Un-debounced Resize Listeners
**Learning:** Attaching complex layout recalculations directly to the `window.addEventListener('resize')` event without debouncing causes severe CPU spikes. Using `requestAnimationFrame` inside the resize listener is insufficient because the resize event fires almost continuously, queuing up a new layout calculation for every frame during the resize action.
**Action:** Always wrap expensive operations bound to the resize event (like masonry grid recalculations reading computed styles) with a `setTimeout` debounce (e.g., 250ms delay) to ensure they only execute once the user has finished resizing the window.
