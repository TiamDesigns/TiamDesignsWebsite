...
## 2024-07-01 - Debounce Three.js resize listeners
**Learning:** Continuous window resize events trigger expensive WebGL reallocations (`renderer.setSize()`), severely blocking the main thread and causing jank. Also, camera projection matrix must be updated on resize when using trackball controls to prevent distortion.
**Action:** Always wrap `window.addEventListener('resize')` for Three.js canvases in a debounce function (e.g., using setTimeout) and remember to call `camera.updateProjectionMatrix()` along with `controls.handleResize()`.
