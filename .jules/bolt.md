...
## 2024-05-25 - Scroll Event Replaced by IntersectionObserver
**Learning:** Even passive, `requestAnimationFrame`-throttled `scroll` event listeners execute their callbacks constantly at 60fps during scroll, wasting CPU cycles on main thread to check values like `window.scrollY`.
**Action:** Replace `scroll` event listeners that toggle classes based on a single scroll threshold with an `IntersectionObserver` monitoring an invisible absolute-positioned sentinel element at the threshold position. This delegates the threshold check to the browser's native compositing thread and only fires the callback precisely when the state changes.
