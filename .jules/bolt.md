...
## 2024-05-24 - Layout Thrashing in Window Resize Event
**Learning:** High-frequency event listeners like `window.addEventListener('resize')` that execute continuously during window dragging can cause significant layout thrashing and high garbage collection overhead if they read computed styles, trigger masonry reflows, or execute memory-intensive Three.js context re-allocations (like `renderer.setSize`).
**Action:** Always wrap expensive resize operations (e.g., grid reflows, canvas sizing, and particle resets) in a debounce function (e.g., using `setTimeout`) to ensure the heavy calculations only fire once the user has stopped dragging the window. When debouncing Three.js resize listeners with `TrackballControls`, make sure to call both `camera.updateProjectionMatrix()` and `controls.handleResize()`.
