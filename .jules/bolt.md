...
## 2024-05-13 - Debounce Expensive Three.js Resize Events
**Learning:** The Three.js `renderer.setSize()` method performs expensive WebGL context reallocations. Running it synchronously on every resize event causes severe main thread blocking and layout thrashing. Furthermore, failing to call `camera.updateProjectionMatrix()` alongside `camera.aspect` updates causes camera distortion on window resize.
**Action:** Always wrap `window.addEventListener('resize')` handlers for Three.js canvases in a debounce function (e.g. `setTimeout`) and remember to call `camera.updateProjectionMatrix()`.
