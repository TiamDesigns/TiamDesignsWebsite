...
## 2024-05-25 - Layout Thrashing and GC Spikes on Resize Event
**Learning:** Binding expensive operations like DOM layout reads (`window.innerHeight`, `document.body.offsetHeight`, `getBoundingClientRect()`), complex grid calculations, and memory-intensive canvas context re-allocations directly to the synchronous, high-frequency `window.addEventListener('resize')` event causes severe continuous layout thrashing and garbage collection spikes.
**Action:** Always wrap expensive or layout-forcing operations bound to the `resize` event in a debounce function (e.g., using `setTimeout`) to ensure they only execute once the layout has settled, thereby preventing continuous frame drops.
