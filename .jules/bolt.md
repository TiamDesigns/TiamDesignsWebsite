...
## 2024-05-26 - Three.js Resize Camera Distortion
**Learning:** Debouncing expensive WebGL `renderer.setSize()` operations on window resize prevents layout thrashing and main thread blocking. However, if the camera aspect ratio is updated (`camera.aspect = width / height`) during this debounced handler without simultaneously calling `camera.updateProjectionMatrix()`, it will result in visual distortion (stretching or squishing) of the 3D model after the resize completes.
**Action:** When debouncing Three.js window resize event listeners, ensure that `camera.updateProjectionMatrix()` is always called immediately after updating `camera.aspect`, and that control updates (e.g., `controls.handleResize()`) are also included in the debounced block to keep the camera view mathematically accurate.
