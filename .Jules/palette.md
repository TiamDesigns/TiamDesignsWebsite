...
## 2026-06-17 - Icon-Only Button Accessibility\n**Learning:** Found an accessibility issue pattern specific to this app's components: icon-only buttons (especially programmatically generated ones like the lightbox controls) used `aria-label` for screen readers, but lacked native `title` attributes, preventing sighted mouse users from seeing tooltips explaining their functions.\n**Action:** Always pair `aria-label` attributes with native `title` attributes for icon-only buttons so both assistive technologies and sighted mouse users receive function descriptions.
