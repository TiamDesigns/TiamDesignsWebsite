...
## 2026-07-05 - Native Tooltips for Icon-Only Buttons
**Learning:** Found an accessibility issue pattern specific to this app's components: icon-only buttons (like `.nav-toggle` and lightbox controls) used `aria-label` for screen readers, but lacked native `title` attributes, meaning sighted mouse users would not receive function descriptions when hovering.
