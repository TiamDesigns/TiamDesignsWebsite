...
## 2024-11-20 - Ensure Tooltips on Icon-Only Buttons
**Learning:** Icon-only buttons (like `.nav-toggle` and lightbox controls) must pair `aria-label` attributes with native `title` attributes so both assistive technologies and sighted mouse users receive function descriptions.
**Action:** Add `title` attributes matching the `aria-label`s on icon-only buttons to improve UX accessibility for all users.
