...
## 2024-07-01 - Title attributes for Icon-Only Programmatic Buttons
**Learning:** Found an accessibility/UX issue pattern specific to this app's components: icon-only buttons created programmatically for the Lightbox (e.g., zoom in, zoom out, close) had `aria-label` attributes for screen readers, but lacked native `title` attributes. This meant sighted mouse users had no tooltips to explain the icon functions.
**Action:** Always pair `aria-label` attributes with native `title` attributes for icon-only buttons (especially those created via JavaScript) so both assistive technologies and sighted mouse users receive function descriptions.
