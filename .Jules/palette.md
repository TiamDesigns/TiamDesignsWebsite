...
## 2024-05-28 - Tooltips for Programmatic Icon-only Buttons
**Learning:** Found an accessibility issue pattern where icon-only buttons (like the Lightbox controls and Nav toggle) had `aria-label` attributes for screen readers but lacked native `title` attributes, leaving sighted mouse users without tooltips to understand their function.
